...
def wait(jid, limit=3000, label=""):
    ...
    while time.time() - t0 < limit:
        ...
        time.sleep(8)
j = post("/api/film/quickstart", {"title": "Night Diner", "look": "photoreal", "resolution": "auto", "place": "night-diner",
                                  "plate": "night_wide", "characters": ["station-master", "tomas-reyl"]})
...
post("/api/film/editscene", {"film": fid, "scene": tree["scenes"][0]["id"], "music": "cinematic, late night diner, lonely saxophone, rain, 58 bpm"})
shots = {}
for s in tree["scenes"][0]["shots"]:
    d = get("/api/film/%s/shot/%s" % (fid, s["id"]))["shot"]; shots[d.get("title", "")] = d
EDITS = {
    "wide - establishing": ("the diner at night, rain on the window, the neon flickering", "", None, ["rain", "light"]),
    "wide - %s and %s" % (A, B): ("sit at the counter, not looking at each other", "", (B, "You're not from around here.", "gruff, friendly"), ["light"]),
    "medium - %s" % A: ("turns his head to look at the man beside him", "look_off", None, ["light"]),
    "medium - %s" % B: ("laughs and shakes his head", "laugh", (B, "Nobody is. That's the point of the place.", "warm"), ["light"]),
    "insert - detail": ("the counter, a coffee cup, steam rising, neon reflected in the chrome", "", None, ["steam", "light"]),
}
...
if os.path.exists(F): subprocess.run(["python3", "/tmp/sheet.py", F, "/tmp/sheet_diner.jpg", "2", "8"])
