...
def main():
    ...
    ap.add_argument("--show", action="store_true",
                    help="print the resolved ComfyUI workflow instead of running it")
    a = ap.parse_args()
    ...
    if a.show:
        # The SAME build path the runner uses, so what you inspect is what would run.
        # A separate "preview" implementation would drift and then lie.
        vals = resolve(dom, sets)
        wf, wf_name = build(dom, vals)
        try:
            set_path(wf, dom["nodes"]["prefix"], "claude-generated/studio_domains/preview")
        except Exception:
            pass
        print(json.dumps({"workflow": wf_name, "resolved": vals, "graph": wf}, indent=2))
        return
