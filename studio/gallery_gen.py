...
#
# `checkpoint` is here because it is the largest single variable in the project and had
# no way to be compared. It is also the only one that changes the SAMPLER SETTINGS as
# well as the prompt - each checkpoint card carries its own steps, cfg and quality tags,
# because comparing two models at a third model's settings measures nothing.
...
def build_prompt(shot, sets, libs, vary_key, vary_val, ck):
    ...
    bits += [(shot.get("desc") or "").strip(),
             ...
             "modern sports anime, cel shading, cinematic", ck.get("quality_tags", Q)]
...
def generate(tpl_id, tpl, shot_i, vary_key, vary_val, libs, dry=False):
    ...
    # Comparing two checkpoints at a third checkpoint's settings measures nothing, so a
    # checkpoint brings its own steps, cfg, sampler and quality tags.
    ck = {}
    if vary_key == "checkpoint":
        ck = libs.get("checkpoints", {}).get(vary_val, {})
    if not ck:
        ck = next((c for c in libs.get("checkpoints", {}).values()
                   if c.get("file") == CKPT), {})
    ckpt = ck.get("file", CKPT)
    steps, cfg = int(ck.get("steps", STEPS)), float(ck.get("cfg", CFG))
    prompt, resolved, look = build_prompt(shot, tpl.get("sets", {}), libs,
                                          vary_key, vary_val, ck)
    ...
    wf = load_wf(WF)
    # node 1 = CheckpointLoaderSimple, node 8 = KSampler. Verified against
    # workflows/22_anime_kf_ipadapter.json rather than guessed.
    set_path(wf, "1.inputs.ckpt_name", ckpt)
    set_path(wf, "8.inputs.steps", steps)
    set_path(wf, "8.inputs.cfg", cfg)
    if ck.get("sampler"):
        set_path(wf, "8.inputs.sampler_name", ck["sampler"])
    ...
    return {
        ...
        "model": ckpt,
        "workflow": WF,
        "steps": steps, "cfg": cfg, "sampler": ck.get("sampler", SAMPLER),
        "size": list(SIZE),
        "grade": grade,
...
def main():
    ...
    # every library a vary can draw from, plus the ones build_prompt reads
    libs = {k: load_dir(k) for k in set(list(VARIES.values())
                                        + ["looks", "emotions", "lighting", "weather"])}
                ...
                for val in sorted(libs[VARIES[k]]):
                    for i in idxs:
                        ...
                        try:
                            ...
                            continue
                        if not rec:
                            # a dry run returns None by design; that is not a failure
                            if not a.dry:
                                failed += 1
                            continue
