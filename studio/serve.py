...
class H(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if p not in ("/api/save", "/api/render", "/api/make", "/api/workflow",
                     ...
                     "/api/film/compose", "/api/film/pin", "/api/film/triage", "/api/film/pinpreview", "/api/film/anchorcheck", "/api/film/coverage", "/api/film/make", "/api/film/makeall", "/api/film/quickstart",
        ...
        if p.startswith("/api/film/"):
            ...
            fn = {"new": fr.new_film, "edit": fr.edit_film, "scene": fr.new_scene,
                  ...
                  "make": fr.make_shot,
                  "makeall": fr.make_all,
                  "quickstart": fr.quickstart}[p[len("/api/film/"):]]
            try:
                body, code = fn(data)
            except KeyError as e:
                return self._send({"error": str(e)}, 404)
